...
void makeCall()
{
  ...
  for(int i=0;i<sizeof(EMERGENCY_PHONE)/sizeof (EMERGENCY_PHONE[0]);i++){
  sim900a.println("ATD"+EMERGENCY_PHONE[i]+";");
  delay(20000); //20 sec delay
  sim900a.println("ATH");
  delay(1000); //1 sec delay
  Serial.println("call made to"+EMERGENCY_PHONE[i]);
  }
}
 ...
 void sendSms(String text)
{
  ...
  delay(1000);
  sim900a.print("AT+CMGS=\""+EMERGENCY_PHONE[i]+"\"\r");
  delay(1000);
  sim900a.print(text);
  delay(100);
  sim900a.write(0x1A); //ascii code for ctrl-26 //sim900a.println((char)26); //ascii code for ctrl-26
  delay(1000);
  Serial.println("SMS Sent Successfully to"+ EMERGENCY_PHONE[i]);
  if (i < sizeof(EMERGENCY_PHONE) / sizeof(EMERGENCY_PHONE[0]) - 1) {
      delay(5000); // 5 sec delay between SMS messages
    }
 }
